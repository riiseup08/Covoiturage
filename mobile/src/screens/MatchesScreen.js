...
export default function MatchesScreen({ navigation }) {
  ...
  const load = async () => {
    try {
      const data = await matchApi.mine();
      setList(data);
    } catch { /* ignore */ }
  };
  ...
  const handleValidate = async (id) => {
    try {
      ...
      Alert.alert('Succès', 'Match validé !');
      load();
    } catch (e) { Alert.alert('Erreur', e.message); }
  };

  const handleRefuse = async (id) => {
    Alert.alert('Confirmer', 'Refuser ce match ?', [
      { text: 'Non' },
      { text: 'Oui', style: 'destructive', onPress: async () => {
        try { await matchApi.refuse(id); load(); } catch (e) { Alert.alert('Erreur', e.message); }
      }},
    ]);
  };
      ...
      <Card>
        <View style={styles.matchHeader}>
          <View style={styles.matchRoute}>
            ...
            <Text style={styles.matchMeta}>
              {isDriver ? '🚗 Vous êtes conducteur' : '🙋 Vous êtes passager'}
              {' · '}{otherUser}
            </Text>
          </View>
        ...
        {status === 'pending' && isDriver && (
          <View style={styles.actions}>
            <Button title="Valider" onPress={() => handleValidate(item.id)} style={{ flex: 1 }} />
            <Button title="Refuser" variant="danger" onPress={() => handleRefuse(item.id)} style={{ flex: 1 }} />
          </View>
        )}
...
function StatusBadge({ status }) {
  const config = {
    ...
    validated: { color: Colors.success, label: 'Validé', icon: 'checkmark-circle' },
    refused: { color: Colors.danger, label: 'Refusé', icon: 'close-circle' },
  };
