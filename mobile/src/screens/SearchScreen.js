...
export default function SearchScreen({ navigation }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const data = await voyages.search(params);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchBox}>
        <Input placeholder="Ville de départ" value={villeDepart} onChangeText={setVilleDepart} style={styles.inputHalf} />
        <Input placeholder="Ville d'arrivée" value={villeArrivee} onChangeText={setVilleArrivee} style={styles.inputHalf} />
        <Button title="Rechercher" onPress={handleSearch} loading={loading} style={{ marginTop: -4 }} />
      </View>
      ...
      {Platform.OS !== 'web' && geoTrips.length > 0 && (
        <TouchableOpacity style={styles.mapToggle} onPress={() => setShowMap(!showMap)}>
          ...
          <Text style={styles.mapToggleText}>{showMap ? 'Voir la liste' : 'Voir la carte'}</Text>
        </TouchableOpacity>
      )}
